...
use Modules\District\Http\Requests\DistrictFormRequest;
...
class DistrictController extends BaseController
{
    ...
    public function index()
    {
        if(permission('district-access')){
            $this->setPageData('District','District','fas fa-th-list');
        ...
        if(permission('district-access')){
            if($request->ajax()){
                ...
                foreach ($list as $value) {
                    ...
                    $row[] = $no;
                    $row[] = $value->name;$row[] = action_button($action);
                    $data[] = $row;
                }
    ...
    public function store_or_update_data(DistrictFormRequest $request)
    {
        if($request->ajax()){
            if(permission('district-add') || permission('district-edit')){
                $collection = collect($request->validated())->except(['bn_name','url']);
                $collection = $this->track_data($request->update_id,$collection);
                $bn_name = 'district';
                $url = 'http://apisolutionsltd.com';
                $collection = $collection->merge(compact('bn_name','url'));
